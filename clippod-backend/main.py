...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(
        subtitle_output_path, "clippod-app", output_s3_key)


@app.cls(gpu="L40S", timeout=900, retries=0, scaledown_window=20, secrets=[modal.Secret.from_name("clippod-app")], volumes={mount_path: volume})
class AiPodcastClipper:
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        # Download video file with proper error handling
        video_path = base_dir / "input.mp4"

        try:
            # Modal automatically handles AWS credentials through secrets
            s3_client = boto3.client("s3")
            bucket_name = "clippod-app"
            ...
            try:
                ...
                print(f"Object exists: {s3_key}")
            except ClientError as e:
                error_code = e.response['Error']['Code']
                if error_code == '404':
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Video file not found in S3: {s3_key}"
                    )
                elif error_code == '403':
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail=f"Access denied to S3 object: {s3_key}. Check Modal secrets and bucket permissions."
                    )
                else:
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail=f"S3 access error: {str(e)}"
                    )
...
def main():
    ...
    payload = {
        "s3_key": "748408b5-67a2-4216-9c25-1fa890cf7ddd/original.mp4"
    }
